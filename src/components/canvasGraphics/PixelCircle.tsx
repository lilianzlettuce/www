...
const DitherCircle = ({
  ...
  easeInFactor = 0.85,
    ...
    }, [width, height, pixelSize, defaultRadius, circleColor, circleEndColor]);
    ...
    const updatePixels = useCallback((mouseX: number, mouseY: number) => {
        ...
        for (const pixel of pixels.current) {
            if (mouseActive.current) {
                ...
                const waveOffset = Math.sin(autoAnimPhase.current + pixel.x * 0.01 + pixel.y * 0.01);
                pixel.radius = pixel.lastRadius + waveOffset * animationRadius.current;
                pixel.radius = Math.max(minRadius, Math.min(maxRadius, pixel.radius));
            }
        }
    ...
    }, [pixels, , circleColor, circleEndColor, debounceTime, attractionDistance, defaultRadius, minRadius, maxRadius, autoAnimStep, easeInFactor]);
